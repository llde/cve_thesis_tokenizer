...
idx = 4
exclude = [0,1,2,3,20,21,22,23,24,25,30,35,40,50,232,233,234,235,236,237,238,239,240,241,42,243,244,245,246,247,248,249,250,251,252,253,254,255]
cache = {}
...
def token_to_pixel(token):
    if(tok[0] == "keyword"):
        ...
        if(tok[1] == "register"): return 233 #register doesn't modify behaviour in modern compilers, ignore?
        if(tok[1] == "sizeof"): return 232 #register doesn't modify behaviour in modern compilers, ignore?
    if(tok[0] == "directive"): return 20
    if(tok[0] == "type"): return 50
    if(tok[0] == "include"): return 21
    if(tok[0] == "string"): return 22
    if(tok[0] == "char" or tok[0] == "continue_char"): return 23
    if(tok[0] == "number"): return 24
    if(tok[0] == "whitespace"): return 1
    if(tok[0] == "nl" or tok[0] == "endline"): return 2
    if(tok[0] == "op"): return 25
    if(tok[0] == "id"):
        ...
        for file in os.scandir(path_pre):
            ...
            for i in range(0,size):
                for j in range(0,size):
                    if(currIndex >= lenght): break
                    tok = tok_code[currIndex]
                    imp[j,i] = token_to_pixel(tok);
